            ...
            Button {
              ...
              LumaCard {
                VStack(alignment: .leading, spacing: 10) {
                  ...
                  if !d.text.isEmpty { Text(d.text).font(LumaStyle.font(14)).lineLimit(4) }
                  if let voices = d.voiceNotes, !voices.isEmpty {
                    Label("Голосовых записей: \(voices.count)", systemImage: "waveform")
                      .font(LumaStyle.font(12)).foregroundStyle(LumaStyle.lavender)
                  }
                }
              }
            }.buttonStyle(.plain)
...
struct SessionFeedbackView: View {
  ...
  @State private var createdClips: [JournalVoice] = []
  @State private var startingVoice = false
  private var voiceBinding: Binding<[JournalVoice]> {
    Binding(get: { feedback.voiceNotes ?? [] }, set: { feedback.voiceNotes = $0 })
  }
  ...
  var body: some View {
    NavigationStack {
      ScrollView {
        VStack(alignment: .leading, spacing: 22) {
          ...
          if session.status == .ended {
            ...
            LumaCard {
              JournalVoiceComposer(audio: model.audio, clips: voiceBinding,
                created: { createdClips.append($0) }, starting: $startingVoice)
            }
            Button("Сохранить отклик") {
              if model.saveFeedback(feedback) { dismiss() }
            }.buttonStyle(PrimaryButtonStyle()).disabled(model.audio.recording || startingVoice)
            if model.state.feedback.contains(where: { $0.sessionID == session.id }) {
              Button("Удалить отклик", role: .destructive) { deleting = true }.frame(
                maxWidth: .infinity, minHeight: 44).disabled(model.audio.recording || startingVoice)
            }
          } else {
      ...
      { ToolbarItem(placement: .cancellationAction) { Button("Закрыть") { dismiss() }.disabled(model.audio.recording || startingVoice) } }
    }
    .interactiveDismissDisabled(model.audio.recording || startingVoice)
    .onDisappear {
      model.removeUnreferencedJournalAudio(createdClips)
      if feedback.voiceNotes?.contains(where: { $0.id == model.audio.playbackID }) == true { model.audio.stopPlayback() }
    }
    .confirmationDialog("Удалить отклик?", isPresented: $deleting, titleVisibility: .visible) {
      Button("Удалить", role: .destructive) {
        if model.deleteFeedback(session.id) { dismiss() }
      }
    }
  }
...
struct DreamEditor: View {
  ...
  @State private var startingVoice = false
  private var voiceBinding: Binding<[JournalVoice]> {
    Binding(get: { dream.voiceNotes ?? [] }, set: { dream.voiceNotes = $0 })
  }
  var body: some View {
    NavigationStack {
      ScrollView {
        VStack(alignment: .leading, spacing: 20) {
          ...
          Text("Запишите голосом, текстом или вместе.").font(LumaStyle.font(13)).foregroundStyle(LumaStyle.secondary)
          TextEditor(text: $dream.text).frame(minHeight: 170).scrollContentBackground(.hidden)
            .padding(12).background(LumaStyle.surface, in: RoundedRectangle(cornerRadius: 20))
          LumaCard {
            JournalVoiceComposer(audio: model.audio, clips: voiceBinding,
              created: { createdClips.append($0) }, starting: $startingVoice)
          }
          Button("Сохранить сон") { if model.saveDream(dream) { dismiss() } }
            .buttonStyle(PrimaryButtonStyle()).disabled(!dream.hasContent || model.audio.recording || startingVoice)
          if model.archive.diary.contains(where: { $0.id == dream.id }) {
            Button("Удалить сон", role: .destructive) { deleting = true }.frame(minHeight: 44)
              .disabled(model.audio.recording || startingVoice)
          }
        }.padding(24)
      }.lumaScreen().toolbar {
        ToolbarItem(placement: .cancellationAction) {
          Button("Закрыть") { dismiss() }.disabled(model.audio.recording || startingVoice)
        }
      }
    }.interactiveDismissDisabled(model.audio.recording || startingVoice)
    .onDisappear {
      model.removeUnreferencedJournalAudio(createdClips)
      if dream.voiceNotes?.contains(where: { $0.id == model.audio.playbackID }) == true { model.audio.stopPlayback() }
    }
    .confirmationDialog("Удалить сон?", isPresented: $deleting, titleVisibility: .visible) {
      Button("Удалить", role: .destructive) { if model.deleteDream(id: dream.id) { dismiss() } }
    }
  }
}
