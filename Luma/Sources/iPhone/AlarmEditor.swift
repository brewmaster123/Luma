          ...
          if alarm.signal.output.phoneEnabled {
            LumaCard {
              VStack(alignment: .leading, spacing: 12) {
                Picker("Тип сигнала", selection: Binding(get: { alarm.delivery }, set: { alarm.phoneDelivery = $0 })) {
                  ForEach(PhoneAlarmDelivery.allCases) { delivery in
                    Text(delivery.title).tag(delivery).disabled(delivery == .systemAlarm && !model.supportsSystemAlarms)
                  }
                }
                Text(alarm.delivery == .systemAlarm
                  ? "Звучит сквозь беззвучный режим и фокусирование, даже когда Luma закрыта. Повтором звука, вибрацией и отображением на Apple Watch управляет iOS; для остановки может потребоваться кнопка «Остановить»."
                  : "Дорожка звучит один раз и затихает сама. Для звука при заблокированном экране в беззвучном режиме оставьте активным ночной сеанс с микрофоном. Обычное уведомление без такого сеанса может быть без звука.")
                  .font(LumaStyle.font(12)).foregroundStyle(LumaStyle.secondary)
              }
            }
          }
          LumaCard { SignalEditor(signal: $alarm.signal, voices: model.state.voices, systemAlarm: alarm.delivery == .systemAlarm) }
