  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 16) {
        ...
        Text("REM — это фаза быстрого сна, во время которой мозг активно работает, а глаза быстро двигаются под веками. В этой фазе часто возникают яркие, эмоциональные и запоминающиеся сны.")
        Text("Осознанным называют сон, в котором человек понимает, что спит. Иногда это позволяет влиять на сюжет. Такие сновидения обычно связывают с REM-фазой.")
        Text("Во время REM тонус большинства скелетных мышц снижен. Это естественное состояние, называемое REM-атонией: тело продолжает спать, пока во сне разворачиваются события.")
        Text("Ближе к утру REM-периоды обычно становятся длиннее. Это оставляет больше времени для сновидений, но не гарантирует, что сон станет осознанным.")
        Text("Apple Watch помогают Luma оценивать вероятность REM по доступным измерениям пульса и движений. Точность зависит от качества и частоты данных. Для Luma она пока не измерена: результат 70–75% и его рост с каждой новой моделью часов обещать нельзя.")
        Text("Режим может работать и только с iPhone, используя признаки звука с микрофона. Без данных часов оценка REM менее надёжна; размер снижения точности ещё не установлен. Для более полного набора данных рекомендуется использовать Apple Watch.")
        Text("Когда ложитесь спать, включите «REM · всю ночь». При каждом новом предполагаемом REM-эпизоде Luma подаёт короткую подсказку: вибрацию на Apple Watch, звук на iPhone или оба сигнала — как выбрано в настройках.")
        Text("Вместо мелодии можно записать собственный голосовой сигнал или выбрать аудио из файлов. Вибрация самого iPhone во время короткой подсказки включается отдельно в настройках.")
        Text("Короткая дорожка проигрывается один раз, а серия подсказок на часах заканчивается сама. Завершать их вручную не нужно. Наблюдение продолжается до завершения сеанса; при нехватке свежих данных сигнал может быть пропущен.")
        Text("Такие подсказки могут помочь заметить, что вы спите, и войти в осознанное сновидение. Эффект индивидуален: многократное повышение шансов для Luma пока не подтверждено.")
        Link("Подробнее о стадиях сна", destination: URL(string: "https://www.nhlbi.nih.gov/health/sleep/stages-of-sleep")!)
          .font(LumaStyle.font(12)).tint(LumaStyle.lavender)
