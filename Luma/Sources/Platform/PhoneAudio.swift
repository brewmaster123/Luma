  ...
  final class PhoneAudio: NSObject, ObservableObject, AVAudioRecorderDelegate, AVAudioPlayerDelegate
  {
    ...
    func notificationSound(_ signal: SignalSettings, clips: [VoiceClip]) throws -> NotificationService.Sound {
      let url = try soundURL(signal, clips: clips)
      let file = try AVAudioFile(forReading: url)
      let duration = Double(file.length) / file.processingFormat.sampleRate
      guard duration.isFinite, duration > 0, duration <= 28 else {
        throw AppError.message("Выберите звук длительностью до 28 секунд.")
      }
      return NotificationService.Sound(name: url.lastPathComponent, duration: duration)
    }
