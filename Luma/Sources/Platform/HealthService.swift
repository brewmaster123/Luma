...
final class HealthService {
    ...
    func requestReadAccess() async throws {
        guard Self.available else { throw AppError.message("Данные «Здоровья» недоступны на этом устройстве.") }
        let types: Set<HKObjectType> = [HKObjectType.quantityType(forIdentifier: .heartRate)!,
                                       HKObjectType.categoryType(forIdentifier: .sleepAnalysis)!]
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            store.requestAuthorization(toShare: [], read: types) { success, error in
                if let error { continuation.resume(throwing: error) }
                else if success { continuation.resume() }
                else { continuation.resume(throwing: AppError.message("Запрос доступа не завершён.")) }
            }
        }
    ...
    func heartSamples(now: Date) async throws -> [HeartSample] {
        ...
        return samples.compactMap { sample in
            guard let value = sample as? HKQuantitySample,
                  ...
                  isWatchSample(value) else { return nil }
            let bpm = value.quantity.doubleValue(for: unit)
            return HeartSample(date: value.endDate, bpm: bpm,
    ...
    func sleepSegments(now: Date) async throws -> [SleepSegment] {
        ...
        let predicate = HKQuery.predicateForSamples(withStart: now.addingTimeInterval(-36 * 3600), end: now)
        return try await query(type: type, predicate: predicate, limit: 2000).compactMap { sample in
            ...
            return SleepSegment(id: value.uuid.uuidString, start: value.startDate, end: value.endDate,
    ...
    func beginObserving() async throws {
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            store.enableBackgroundDelivery(for: type, frequency: .hourly) { success, error in
                ...
                else { continuation.resume(throwing: AppError.message("Фоновые обновления «Здоровья» недоступны.")) }
            }
        }
    }

    private func query(type: HKSampleType, predicate: NSPredicate?, limit: Int) async throws -> [HKSample] {
