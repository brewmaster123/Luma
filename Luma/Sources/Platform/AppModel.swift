...
@MainActor final class AppModel: ObservableObject {
  ...
  var sortedAlarms: [LumaAlarm] { AlarmPlanner.sorted(state.alarms, after: alarmOrderDate) }
    ...
    var newAlarmDelivery: PhoneAlarmDelivery { state.defaultPhoneDelivery ?? (phoneAlarms.supported ? .systemAlarm : .shortCue) }
    var shortPhoneAlarmCount: Int { state.alarms.filter { $0.signal.output.phoneEnabled && $0.delivery == .shortCue }.count }
    func setPhoneVibration(_ enabled: Bool) { let old = state; state.phoneVibration = enabled; if !save() { state = old } }
    func setDefaultDelivery(_ delivery: PhoneAlarmDelivery) { let old = state; state.defaultPhoneDelivery = delivery; if !save() { state = old } }
    func enableSystemAlarmsForExisting() async {
      guard !busy else { return }; busy = true; defer { busy = false }
      do {
        try await phoneAlarms.requestPermission()
        let old = state
        for i in state.alarms.indices where state.alarms[i].signal.output.phoneEnabled { state.alarms[i].phoneDelivery = .systemAlarm }
        state.defaultPhoneDelivery = .systemAlarm
        guard save() else { state = old; return }
        await rebuildAlarms(); syncConfiguration()
  ...
  @discardableResult func saveAlarm(_ input: LumaAlarm) async -> Bool {
    ...
    if alarm.enabled && alarm.signal.output.phoneEnabled {
      do {
        _ = try audio.soundURL(alarm.signal, clips: state.voices)
        if alarm.delivery == .systemAlarm { try await phoneAlarms.requestPermission() }
        else { try await notifications.requestPermission() }
  ...
  func preview() async {
    #if os(iOS)
      do {
        if state.signal.output.phoneEnabled { try audio.play(state.signal, clips: state.voices, vibrate: state.vibratesOnPhone) }
  ...
  func refreshSensors() async {
    ...
    let now = Date()
      ...
      if session.signal.output.phoneEnabled {
        do {
          try audio.play(session.signal, clips: state.voices, vibrate: state.vibratesOnPhone)
          result(e.id, phone: "Воспроизведение запущено; беззвучный режим не подавляет дорожку")
  ...
  func deleteHistory() {
    ...
    private func handleAlarm(_ id: String) -> Bool {
      ...
      do {
        try audio.play(a.signal, clips: state.voices, vibrate: state.vibratesOnPhone)
        excludedUntil = Date().addingTimeInterval(120)
        resetStreak()
      } catch { message = error.localizedDescription }
  ...
  private func rebuildAlarms() async {
    ...
    repeat {
      rebuildAgain = false
      #if os(iOS)
      let queued = state.alarms.filter { $0.delivery == .shortCue || !$0.signal.output.phoneEnabled }
      #else
      ...
      #if os(iOS)
      let system = await phoneAlarms.rebuild(state.alarms) { try audio.soundURL($0, clips: state.voices).lastPathComponent }
  ...
  func syncConfiguration() {
      ...
      p.revision = state.configurationRevision
      p.alarms = state.alarms.filter { ids.contains($0.id) && !($0.delivery == .systemAlarm && $0.signal.output.phoneEnabled) }
