...
final class FiniteHaptics {
    ...
    func preview(count: Int) -> String? {
        ...
        let token = UUID(); generation = token; isPlaying = true
        task = Task { @MainActor [weak self] in
            defer { if self?.generation == token { self?.isPlaying = false; self?.task = nil } }
    ...
    func cancel() { generation = UUID(); task?.cancel(); task = nil; isPlaying = false }
}
