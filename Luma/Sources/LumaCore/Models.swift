...
public enum CueCount: Int, Codable, CaseIterable, Identifiable, Sendable {
    case one = 1, two = 2, three = 3, five = 5
...
public struct NightSettings: Codable, Equatable, Sendable {
    ...
    public func nextPlan(now: Date, calendar: Calendar = .current) -> NightPlan? {
        ...
        return NightPlan(createdAt: now, windowStart: start,
...
public struct NightPlan: Codable, Equatable, Identifiable, Sendable {
    ...
    public init(id: UUID = UUID(), createdAt: Date, windowStart: Date, windowEnd: Date, settings: NightSettings) {
        self.id = id; self.createdAt = createdAt; self.windowStart = windowStart
        self.windowEnd = windowEnd; self.settings = settings.validated()
    }
...
public struct HeartSample: Codable, Equatable, Sendable {
    ...
    public init(date: Date, bpm: Double, source: String = "watch") {
        self.date = date; self.bpm = bpm; self.source = source
    }
}
...
public struct MotionEpoch: Codable, Equatable, Sendable {
    ...
    public init(start: Date, end: Date, rmsG: Double, sampleCount: Int) {
        self.start = start; self.end = end; self.rmsG = rmsG; self.sampleCount = sampleCount
    }
}
...
public struct REMEvidence: Codable, Equatable, Sendable {
    ...
    public init(kind: EvidenceKind, heuristicScore: Double? = nil, latestSampleAt: Date? = nil, explanation: String) {
        self.kind = kind; self.heuristicScore = heuristicScore
        self.latestSampleAt = latestSampleAt; self.explanation = explanation
    }
...
public struct CueRecord: Codable, Equatable, Identifiable, Sendable {
    ...
    public init(request: CueRequest, status: CueStatus = .reserved, note: String = "") {
        id = request.id; planID = request.planID; scheduledAt = request.fireAt
...
public struct SleepSegment: Codable, Equatable, Identifiable, Sendable {
    ...
    public var source: String
    public init(id: String = UUID().uuidString, start: Date, end: Date, stage: SleepStage, source: String) {
...
public struct SyncPacket: Codable, Identifiable, Sendable {
    ...
    public init(kind: PacketKind, plan: NightPlan? = nil, replyTo: UUID? = nil,
                planState: PlanState? = nil, message: String? = nil, records: [CueRecord]? = nil) {
        protocolVersion = 1; id = UUID(); sentAt = Date(); self.kind = kind; self.plan = plan
        self.replyTo = replyTo; self.planState = planState; self.message = message; self.records = records
    }
}
