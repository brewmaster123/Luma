...
public struct REMEstimator: Sendable {
    ...
    public func evaluate(heart: [HeartSample], motion: [MotionEpoch], now: Date) -> REMEvidence {
        ...
        guard let series = grouped.values.max(by: { a, b in
        ...
        guard let first = recent.first, let last = recent.last,
              recent.count >= 6, last.date.timeIntervalSince(first.date) >= 6 * 60 else {
        ...
        guard zip(recent, recent.dropFirst()).allSatisfy({ $1.date.timeIntervalSince($0.date) <= 150 }) else {
        ...
        guard baseline.count >= 6, let b0 = baseline.first, let b1 = baseline.last,
              b1.date.timeIntervalSince(b0.date) >= 10 * 60 else {
        ...
        let epochs = motion.filter {
            ...
            $0.end.timeIntervalSince($0.start) <= 45 && $0.rmsG.isFinite && $0.rmsG >= 0 && $0.sampleCount >= 50
        ...
        guard epochs.count >= 3, let mLast = epochs.last, now.timeIntervalSince(mLast.end) <= 45,
              ...
              epochs.reduce(0.0, { $0 + $1.end.timeIntervalSince($1.start) }) >= 75 else {
        ...
        return REMEvidence(kind: candidate ? .candidate : .unlikely,
    ...
    private func median(_ numbers: [Double]) -> Double {
        let sorted = numbers.sorted(); let middle = sorted.count / 2
