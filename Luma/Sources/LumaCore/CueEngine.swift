...
public struct CueEngine: Sendable {
  ...
  public func decide(
    plan: NightPlan, evidence: REMEvidence, now: Date,
    memory: inout DecisionMemory
  ) -> [CueRequest] {
    guard plan.settings.mode == .experimentalREM, plan.contains(now) else {
      memory.candidateSince = nil
      return []
    }
    if memory.planID != plan.id {
      memory = DecisionMemory()
      memory.planID = plan.id
    }
    guard !memory.reservedBurst else { return [] }
    guard evidence.kind == .candidate, let sampleAt = evidence.latestSampleAt,
      sampleAt <= now, now.timeIntervalSince(sampleAt) <= 90
    else {
      memory.candidateSince = nil
    ...
    if let previous = memory.lastEvaluationAt,
      now < previous || now.timeIntervalSince(previous) > 90
    {
      memory.candidateSince = nil
    }
    memory.lastEvaluationAt = now
    ...
    if memory.candidateSince == nil {
      memory.candidateSince = now
      return []
    }
    guard let since = memory.candidateSince, now.timeIntervalSince(since) >= 120,
      let requests = try? makeRequests(plan: plan, firstAt: now.addingTimeInterval(3))
    else { return [] }
  ...
  private func makeRequests(plan: NightPlan, firstAt: Date) throws -> [CueRequest] {
    ...
    guard firstAt >= plan.windowStart,
      firstAt.addingTimeInterval(Double(count - 1) * gap) < plan.windowEnd
    else {
      throw CuePlanError.notEnoughTime
    }
    return (0..<count).map { index in
      CueRequest(
        id: "luma.\(plan.id.uuidString).\(index)", planID: plan.id,
...
public struct SleepComparison: Sendable {
  ...
  public func compare(record: CueRecord, segments: [SleepSegment]) -> ComparisonResult {
    ...
    guard stages.count == 1, let stage = stages.first, stage != .unspecified else {
      return .unknown
    }
