...
final class CoreSafetyTests: XCTestCase {
  ...
  private func plan(mode: NightMode = .experimentalREM, count: CueCount = .two) -> NightPlan {
    ...
    settings.mode = mode
    settings.cueCount = count
    return NightPlan(
      createdAt: origin.addingTimeInterval(-3600), windowStart: origin,
      windowEnd: origin.addingTimeInterval(1800), settings: settings)
  }
  private func candidate(at date: Date) -> REMEvidence {
    REMEvidence(
      kind: .candidate, heuristicScore: 0.9, latestSampleAt: date, explanation: "Synthetic test")
  }
  func testEveryAllowedCountProducesExactlyThatManyFiniteUniqueRequests() throws {
    for count in CueCount.allCases {
      ...
      XCTAssertEqual(
        requests, try CueEngine().scheduledRequests(plan: p, now: origin.addingTimeInterval(-30)))
    }
  }
  func testLateScheduledCommandNeverReschedulesOrFiresImmediately() {
    XCTAssertThrowsError(
      try CueEngine().scheduledRequests(
        plan: plan(mode: .scheduled), now: origin.addingTimeInterval(10)))
  }
  func testReevaluatingOneMeasurementDoesNotCreateAStreak() {
    ...
    for second in stride(from: 0, through: 240, by: 30) {
      XCTAssertTrue(
        engine.decide(
          plan: p, evidence: candidate(at: origin),
          now: origin.addingTimeInterval(Double(second)), memory: &memory
        ).isEmpty)
    }
  ...
  func testStaleUnknownAndFutureEvidenceNeverTriggers() {
    ...
    let p = plan()
    for evidence in [
      candidate(at: origin.addingTimeInterval(-300)),
      ...
      for second in [0.0, 60, 120, 180] {
        XCTAssertTrue(
          engine.decide(
            plan: p, evidence: evidence, now: origin.addingTimeInterval(second), memory: &memory
          ).isEmpty)
      }
    }
  }
  func testOnlyOneBurstSurvivesSerializationAndRepeatedUpdates() throws {
    ...
    for second in stride(from: 180.0, through: 1500, by: 60) {
      let date = origin.addingTimeInterval(second)
      XCTAssertTrue(
        engine.decide(plan: p, evidence: candidate(at: date), now: date, memory: &memory).isEmpty)
    }
  }
  ...
  func testEstimatorAbstainsWithMissingMotionDespiteLotsOfHeartData() {
    let samples = stride(from: -2700.0, through: 0, by: 60).map {
      HeartSample(date: origin.addingTimeInterval($0), bpm: 65)
    }
    XCTAssertEqual(
      REMEstimator().evaluate(heart: samples, motion: [], now: origin).kind, .insufficient)
  }
  func testDuplicatesAndNonfiniteValuesCannotManufactureEnoughMeasurements() {
    let samples =
      Array(repeating: HeartSample(date: origin, bpm: 65), count: 200) + [
  ...
  func testMixingSensorsCannotManufactureOneDenseStream() {
    let samples = (0..<10).map {
      HeartSample(
        date: origin.addingTimeInterval(Double(-$0 * 60)), bpm: 65, source: "sensor-\($0)")
    }
    XCTAssertEqual(
      REMEstimator().evaluate(heart: samples, motion: [], now: origin).kind, .insufficient)
  }
  func testScheduledTimeIsNotProofOfDeliveryOrREM() throws {
    let request = try XCTUnwrap(
      CueEngine().scheduledRequests(
        plan: plan(mode: .scheduled), now: origin.addingTimeInterval(-60)
      ).first)
    let record = CueRecord(request: request, status: .scheduled)
    let rem = SleepSegment(
      start: origin.addingTimeInterval(-10), end: origin.addingTimeInterval(600), stage: .rem,
      source: "Apple")
  ...
  func testConflictingHistoricalStagesAreUnknown() throws {
    ...
    var record = CueRecord(request: request, status: .deliveryObserved)
    record.deliveredAt = origin
    let rem = SleepSegment(
      start: origin, end: origin.addingTimeInterval(600), stage: .rem, source: "A")
    let core = SleepSegment(
      start: origin, end: origin.addingTimeInterval(600), stage: .core, source: "B")
  ...
  func testNextPlanHandlesMidnightAndDSTWithoutFixed86400Assumption() throws {
    ...
    let now = try XCTUnwrap(
      calendar.date(from: DateComponents(year: 2026, month: 3, day: 8, hour: 0)))
    var settings = NightSettings()
    settings.startHour = 2
