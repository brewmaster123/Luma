...
check('repeats: false' in notifications and 'repeats: true' not in notifications,'Notification triggers are non-repeating in source')
core=(ROOT/'Tests/LumaCoreTests/CoreSafetyTests.swift').read_text()
check(len(re.findall(r'func test\w+\(',core))==13,'13 XCTest scenarios are present (not executed by this script)')
...
ET.parse(ROOT/'Design/Luma-design.svg')
check(True,'Design SVG parses as XML')
